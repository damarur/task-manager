...
import es.damarur.task.manager.repository.TaskEntitySpects;
...
public class TaskManagerServiceImpl implements TaskManagerService {
	...
	public TaskDTO updateTask(TaskDTO taskDTO) throws TaskNotFoundException {
		Optional<TaskEntity> entity = repository.findById(taskDTO.getId());
		if (entity.isPresent()) {
	...
	public boolean finishTask(Integer id) throws BusinessException {
		int count = repository.countTaskEntityByIdAndFinished(id, true);
		if (count == 1) {
			throw new BusinessException("task.already.finished");
		}
		int result = repository.updateFinishedTask(true, new Date(), id);
		return result > 0;
	}

	@Override
	public List<TaskDTO> searchTasks(String title, Boolean finished, Date from, Date to, Pageable pageable) {
		Specification<TaskEntity> spec1 = TaskEntitySpects.finishDateGreaterThan(from);
		Specification<TaskEntity> spec2 = TaskEntitySpects.finishDateLessThan(to);
		Specification<TaskEntity> spec3 = TaskEntitySpects.startDateGreaterThan(from);
		Specification<TaskEntity> spec4 = TaskEntitySpects.startDateLessThan(to);
		Specification<TaskEntity> spec5 = TaskEntitySpects.targetDateGreaterThan(from);
		Specification<TaskEntity> spec6 = TaskEntitySpects.targetDateLessThan(to);
		Specification<TaskEntity> spec7 = TaskEntitySpects.finishedEquals(finished);
		Specification<TaskEntity> spec8 = TaskEntitySpects.titleLike(title);
